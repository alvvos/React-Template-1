...
const Navbar = ({ scrollProgress, isMobile }) => {
  ...
  return (
    <>
        ...
        <div className="container mx-auto px-6 flex justify-between items-center">
          <div
            className={`transition-all duration-1000 ${
              showContent ? "opacity-100" : "opacity-0 translate-y-2"
            }`}
            onClick={scrollToTop}
          >
          ...
          <div
            className={`hidden md:flex space-x-8 items-center font-Garet transition-opacity duration-700 ${
          ...
          <div className="md:hidden">
            ...
            >
              <div
                className={`w-7 flex flex-col items-center space-y-1.5 transition-all duration-400 ${
                  menuOpen ? "rotate-180" : ""
                }`}
              >
                <span
                  className={`block h-[2px] w-6 bg-white transition-all duration-400 ${
                    menuOpen ? "rotate-45 translate-y-[9px] w-7 bg-accent" : ""
                  }`}
                ></span>
                <span
                  className={`block h-[2px] w-5 bg-white transition-all duration-200 ${
                    menuOpen ? "opacity-0" : ""
                  }`}
                ></span>
                <span
                  className={`block h-[2px] w-4 bg-white transition-all duration-400 ${
                    menuOpen
                      ? "-rotate-45 -translate-y-[9px] w-7 bg-accent"
                      : ""
                  }`}
                ></span>
              </div>
      ...
      <div
        className={`md:hidden fixed top-0 right-0 h-full w-72 bg-gradient-to-b from-gray-900 to-gray-800 backdrop-blur-xl z-40 transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] transform ${
          menuOpen ? "translate-x-0" : "translate-x-full"
        } shadow-[0_0_30px_10px_rgba(0,0,0,0.3)] border-l border-gray-700/50`}
        style={{
      ...
      >
        <div className="flex flex-col items-start justify-start h-full pt-12 pl-10 space-y-8">
          {[
            ...
            <button
              key={item.id}
              onClick={() => handleLinkClick(item.id)}
              className={`text-2xl font-medium text-white/90 hover:text-accent transition-all duration-500 transform ${
                menuOpen
        ...
        <div
          className={`absolute bottom-8 left-10 right-10 transition-all duration-700 ${
